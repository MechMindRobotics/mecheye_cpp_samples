...
bool loadCalibration(mmind::eye::ProfilerCalibrationInterfaces& calibInstance, const std::string& prompt);

/**
 * Computes the combined transformation matrix by multiplying two input transformation matrices.
 * The multiplication follows OpenCV's matrix multiplication order (baseTransform * additionalTransform).
 *
 * @param baseTransform        The first transformation matrix (typically representing the base frame).
