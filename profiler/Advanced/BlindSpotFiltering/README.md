# BlindSpotFiltering Sample

With this sample, you can detect and remove the false data caused by blind spots and obtain the filtered profile data.

>Note: The blind spot filtering function can only be applied after all profiles have been retrieved. Therefore, the profile data must be retrieved with a callback function instead of polling.
...
3. Run Cmake and set the source and build paths:
   ...
   | Field                       | Path                         |
   | :----                       | :----                        |
   | Where is the source code    | xxx/BlindSpotFiltering       |
   | Where to build the binaries | xxx/BlindSpotFiltering/build |
...
1. Navigate to the directory of the sample.
   ...
   ```bash
   cd xxx/profiler/BlindSpotFiltering/
   ```
...
3. Run the sample.
   ...
   ```bash
   sudo ./BlindSpotFiltering
   ```

4. Enter the index of the laser profiler to which you want to connect, and press the Enter key. The obtained files are saved to `/BlindSpotFiltering/build`.
