...
With this sample, you can periodically trigger data acquisition with signals input from an external device at a fixed scan rate to warm up the Mech-Eye Profiler, while the acquired data is used only for stabilization and not saved.
...
* [CMake (version 3.5 or above)](https://cmake.org/download/)
...
3. Run Cmake and set the source and build paths:
   ...
   | Field                       | Path                                            |
   | :----                       | :----                                           |
   | Where is the source code    | xxx/WarmUp       |
   | Where to build the binaries | xxx/WarmUp/build |
...
4. Enter the index of the laser profiler to which you want to connect, and press the Enter key. The obtained files are saved to `/WarmUp/build`.
