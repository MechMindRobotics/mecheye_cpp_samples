...
>Note: The noise removal function can only be applied after all profiles have been retrieved. Therefore, the profile data must be retrieved with a callback function instead of polling.
...
3. Run Cmake and set the source and build paths:
   ...
   | Field                       | Path                   |
   | :----                       | :----                  |
   | Where is the source code    | xxx/NoiseRemoval       |
   | Where to build the binaries | xxx/NoiseRemoval/build |
...
4. Enter the index of the laser profiler to which you want to connect, and press the Enter key. The obtained files are saved to `/NoiseRemoval/build`.
