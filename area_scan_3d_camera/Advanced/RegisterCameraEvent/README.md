...
2. Run Cmake and set the source and build paths:
   ...
   | Field                       | Path                          |
   | :----                       | :----                         |
   | Where is the source code    | xxx/RegisterCameraEvent       |
   | Where to build the binaries | xxx/RegisterCameraEvent/build |
...
3. Run the sample.
   ...
   ```bash
   sudo ./RegisterCameraEvent
   ```

4. Enter the index of the camera to which you want to connect, and press the Enter key. The obtained files are saved to `/RegisterCameraEvent/build`.
