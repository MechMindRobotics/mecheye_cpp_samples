...
With this sample, you can obtain and save the point clouds in the custom reference frame.
...
2. Run Cmake and set the source and build paths:
   ...
   | Field                       | Path                          |
   | :----                       | :----                         |
   | Where is the source code    | xxx/TransformPointCloud       |
   | Where to build the binaries | xxx/TransformPointCloud/build |
...
1. Navigate to the directory of the sample.
   ...
   ```bash
   cd xxx/area_scan_3d_camera/Advanced/TransformPointCloud/
   ```
...
3. Run the sample.
   ...
   ```bash
   sudo ./TransformPointCloud
   ```

4. Enter the index of the camera to which you want to connect, and press the Enter key. The obtained files are saved to `/TransformPointCloud/build`.
