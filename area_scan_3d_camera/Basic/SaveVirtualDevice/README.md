...
3. Run Cmake and set the source and build paths:
   ...
   | Field                       | Path                     |
   | :----                       | :----                    |
   | Where is the source code    | xxx/SaveVirtualDevice       |
   | Where to build the binaries | xxx/SaveVirtualDevice/build |
...
1. Navigate to the directory of the sample.
   ...
   ```bash
   cd xxx/area_scan_3d_camera/Basic/SaveVirtualDevice/
   ```
...
3. Run the sample.
   ...
   ```bash
   sudo ./SaveVirtualDevice
   ```

4. Enter the index of the camera to which you want to connect, and press the Enter key. The obtained files are saved to `/SaveVirtualDevice/build`.
