...
2. Run Cmake and set the source and build paths:
   ...
   | Field                       | Path                           |
   | :----                       | :----                          |
   | Where is the source code    | xxx/CapturePointCloudHDR       |
   | Where to build the binaries | xxx/CapturePointCloudHDR/build |
...
1. Navigate to the directory of the sample.
   ...
   ```bash
   cd xxx/area_scan_3d_camera/Basic/CapturePointCloudHDR/
   ```
...
3. Run the sample.
   ...
   ```bash
   sudo ./CapturePointCloudHDR
   ```

4. Enter the index of the camera to which you want to connect, and press the Enter key. The obtained files are saved to `/CapturePointCloudHDR/build`.
